...
public class BroadlinkRemoteHandler extends BroadlinkBaseThingHandler {
    ...
    protected void sendCode(final byte[] code) {
    ...
    public void handleCommand(final ChannelUID channelUID, final Command command) {
        if (command == null) {
            ...
            if (this.logger.isDebugEnabled()) {
                this.logger.debug("Can't handle command {} because handler for thing {} is not ONLINE", command,
            ...
            switch (channel.getChannelTypeUID().getId()) {
                case "command": {
                    if (this.logger.isDebugEnabled()) {
                        this.logger.debug("Handling ir/rf command {} on channel {} of thing {}",
                                new Object[] { command, channelUID.getId(), this.getThing().getLabel() });
                    }
                ...
                default:
                    if (this.logger.isDebugEnabled()) {
                        this.logger.debug("Thing {} has unknown channel type {}", this.getThing().getLabel(),
    ...
    private byte[] lookupCode(final Command command, final ChannelUID channelUID) {
        if (command.toString() == null) {
            ...
            if (StringUtils.isEmpty(mapFile)) {
                if (this.logger.isDebugEnabled()) {
                    this.logger.debug("MAP file is not defined in configuration of thing {}",
                            this.getThing().getLabel());
                }

                return null;
            } else {
                final TransformationService transformService = TransformationHelper
                        .getTransformationService(this.bundleContext, "MAP");
                if (transformService == null) {
                    this.logger.error("Failed to get MAP transformation service for thing {}; is bundle installed?",
                            this.getThing().getLabel());
                    return null;
                } else {
                    String value;
                    ...
                    try {
                        ...
                        this.logger.error("Failed to transform {} for thing {} using map file '{}', exception={}",
                                new Object[] { command, this.getThing().getLabel(), mapFile, e.getMessage() });
                        return null;
                    }

                    if (StringUtils.isEmpty(value)) {
                        this.logger.error("No entry for {} in map file '{}' for thing {}",
                                new Object[] { command, mapFile, this.getThing().getLabel() });
                        return null;
                    } else {
                        if (this.logger.isDebugEnabled()) {
                            this.logger.debug("Transformed {} for thing {} with map file '{}'",
                                    new Object[] { command, this.getThing().getLabel(), mapFile });
                        }
